...
While localization in an auto has very little to no drift, that is just 30 seconds. 2 minutes of Teleop can have a bit of drift. 
You can update your position using `follower.setPose`. 

Only use this if you either have a way to relocalise (eg. a LimeLight or a camera pipeline), or you know your position (eg. in a corner). 

Lets add a way to relocalise in a corner to our tele-op so far
```java
@Override
public void loop() {
    DrivePowers powers = ManualDrive.fieldCentric(
    ...
    telemetry.addData("Robot Y", robotPose.y());
    telemetry.addData("Robot Heading", Math.toDegrees(robotPose.heading())); 
    // Math.toDegrees() is a build in java method
}
```
This code will set our robot's position to a corner when we press "Start".

### Passing your position from Autonomous to Tele-OP
Pedro doesn't automatically save your position during Tele-OP. You have to send it from your Autonomous to your Tele-OP manually. Pedro doesn't have a built in way for this, but there are several ways you can do this. For example, a storage class.

Create a new class, we will call it OpModeStorage: 
...
This should be accessible by your autos and tele-ops. So ideally not in any folder
...
Now back in your Tele-OP code, add this code in your `start()` method
...
Afterwards your Tele-OP will set its starting position to what it finds in OpModeStorage

You can use this same method to save any other values (eg, states, patterns, etc), from your autonomous and put it into your Tele-OP
