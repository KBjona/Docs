...
### Follower String
The `Follower` also has its own debug string for the `FollowerLog` that contains all of the debug maps and additional data.

You may access the follower string with the `follower.debug().toString()` method.

<Callout title="Tip" type="info">
    Alternatively, if you call `follower.withLogger(log -> telemetry.addData("Follower Log", log));` once in `start` or `init`, the `Follower` will automatically print its debug string to telemetry.
