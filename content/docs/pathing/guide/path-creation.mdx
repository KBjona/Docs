...
Now that we have already created two `Pose`s, we can create a `path` between them.
...
<Callout title="Tip" type="info">
    Paths default to **tangential** (robot faces the direction it is driving) heading if you don't add any interpolation. See [Interpolation](/docs/pathing/reference/interpolation) for the available options.
</Callout>
